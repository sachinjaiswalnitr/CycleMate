...
function Borrow_cycle() {
  ...
  const selectCycle = async () => {
    ...
    try {
      ...
      if (same.length > 0) {
        ...
        toast.error("No cycle available to buy at the moment.");
        setLoading(false);
        return;
      }
  ...
  const handleBorrow = async () => {
    ...
    try {
      ...
      toast.success("Cycle bought successfully");
      context.buyCycle();
      setHostel("");
      setReturnDate("");
      setReason("");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#bce0dc] to-[#c3e7e3] py-20 px-4 relative overflow-hidden">
      ...
      <div className="relative max-w-3xl mx-auto bg-[#e6f2f1] shadow-2xl rounded-3xl p-8 z-10">
        <h1 className="text-3xl font-bold text-center text-[#007C7C] mb-6">Buy Cycle </h1>
