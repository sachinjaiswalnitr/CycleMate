  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#bce0dc] to-[#c3e7e3] py-20 px-4 relative overflow-hidden">
      <svg className="absolute top-0 left-0 w-full h-[50vh] z-0" viewBox="0 0 1440 320" preserveAspectRatio="none">
        <path fill="#004f47" fillOpacity="1" d="M0,160 C360,320 1080,0 1440,160 L1440,0 L0,0 Z" />
      </svg>
      ...
      <div className="relative bg-[#e6f2f1] shadow-2xl rounded-3xl p-10 max-w-2xl w-full mx-auto z-10">
        ...
        <form onSubmit={handleOnSubmit} className="space-y-6">
          {[{ label: 'Cycle ID', name: 'cycleID', type: 'text', placeholder: 'Enter Cycle ID' },
            { label: 'Issue Description', name: 'issue', type: 'textarea', placeholder: 'Describe the issue' }]
          ...
          <div>
            ...
            >
              ...
              {["VS Hall", "SD Hall", "MV Hall", "GDB Hall", "DBA Hall", "MSS Hall", "HB Hall", "BF Hall", "KMS Hall", "CVR Hall"]
        ...
        {visTime && (
          <div className="mt-8 p-6 rounded-2xl bg-gray-100 shadow-inner">
            ...
            <p><strong>Cycle ID:</strong> {fault.cycleID}</p>
            <p><strong>Hostel:</strong> {fault.hostel}</p>
            <p><strong>Issue:</strong> {fault.issue}</p>
            <p><strong>Visit By:</strong> {visTime}</p>
          </div>
        )}
